...
def main(UDS,distance=50):
    ...
    abdf = []
    for BF in UDS['Features']['BuildingFootprints']:
        try:
            abdf.append({
    ...
    for i,row in abdf.iterrows():
        try:
            ...
            UDS['Features']['BuildingFootprints'][i]['properties']['ViewObstruction'] = sum(ViewObstructionCount)
            total_score += sum(ViewObstructionCount)
        except Exception as e: 
            print(e)
    UDS['Evaluations']['TotalViewObstruction'] = total_score
    return UDS
