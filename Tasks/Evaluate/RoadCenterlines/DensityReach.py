...
def main(UDS):
    ...
    for i in range(len(UDS['Phenotype']['RoadCenterlines'])):
        UDS['Phenotype']['RoadCenterlines'][i]['properties']['DensityReach'] = 0
    ...
    for BG in UDS['Phenotype']['ParcelBoundaries']:
        ...
        if GPR:
            for target_index in target_indicies:
                ...
                for spi, epi in zip(shortest_path_node_index[:-1],shortest_path_node_index[1:]): 
                    ...
                    UDS['Phenotype']['RoadCenterlines'][path_segment_properties['edge_index']]['properties']['DensityReach'] += 0.001 * float(GPR) * float(path_segment_properties['category']) * float(path_segment_properties['length'])

    UDS['Genotype']['TotalDensityReach'] = sum([RC['properties']['DensityReach'] for RC in UDS['Phenotype']['RoadCenterlines']])
    return UDS
