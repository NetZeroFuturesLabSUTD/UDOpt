...
def RemovePhenotypeWithinBoundary(UDS, reference, always_include=['StudyArea']):
    ...
    phenotype_copy = {}
    include, exclude = 0,0
    for key,phenotypes in UDS['Features'].items():
    ...
    ans['Features'] = phenotype_copy
    return ans

def IncludeUDSWithinBoundaries(UDS, boundaries):
    UDS_include = copy.deepcopy(UDS)
    UDS_include['Features'] = {}
    UDS_exclude = copy.deepcopy(UDS)
    UDS_exclude['Features'] = {}

    for key,phenotypes in UDS['Features'].items():

        if key not in UDS_include['Features']: 
            UDS_include['Features'][key] = []
        if key not in UDS_exclude['Features']: 
            UDS_exclude['Features'][key] = []

        for phenotype in phenotypes:
            condition = [WithinBoundaryBuffer(shape(phenotype['geometry']), boundary) for boundary in boundaries]
            if any(condition):
                UDS_include['Features'][key].append(phenotype)
            else:
                UDS_exclude['Features'][key].append(phenotype)
...
def GetGFA(UDS):
    GFA = []
    siteArea = []
    if not UDS['Features']['ParcelBoundaries']:
        print('No ParcelBoundaries')
        return None

    for PB in UDS['Features']['ParcelBoundaries']:
        try:
            inside_UDS, outside_UDS = IncludeUDSWithinBoundaries(UDS, boundaries=[shape(PB['geometry'])])
            siteArea.append(shape(PB['geometry']).area)
            if len(inside_UDS['Features']['BuildingFootprints']) == 0:
                GFA.append(np.nan)
            else:
                GFA.append(sum([shape(BF['geometry']).area * BF['properties']['levels'] for BF in inside_UDS['Features']['BuildingFootprints']]))  
        except:
            pass
