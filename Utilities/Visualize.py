    ...
    for i in range(len(UDSs)):
        ...
        for phenotype in UDS['Features']:
            ...
            # else plot it
                ...
                geometries = [shape(node['geometry']) for node in UDS['Features'][phenotype]]
...
def PlotUDSMetric(UDS, figSize=(14,8), labelMetrics={}, legend=True):
    ...
    for phenotype_name in sorted(UDS['Features'])[::-1]:

        color_phenotypes = []
        plain_phenotypes = []

        if phenotype_name=='StudyAreas':
            gdf = gpd.GeoDataFrame({'geometry': [shape(SA['geometry']) for SA in UDS['Features']['StudyAreas']]})
        ...
        if not labelMetrics or phenotype_name not in labelMetrics or phenotype_name!='StudyAreas':
            for phenotype in UDS['Features'][phenotype_name]:
